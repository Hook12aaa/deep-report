...
Dispatch the `figure-spec-author` subagent (`agents/figure-spec-author.md`) for each draft section that warrants a figure. The subagent runs with fresh context, picks one of four families (`hierarchy`, `sequence`, `matrix`, `table`) or refuses with a tagged reason, and emits a JSON spec conforming to `schemas/figure-spec.schema.json`. The subagent never emits SVG, Mermaid, or DOT. Write each accepted spec to `report/figures/<figure-id>.spec.json`.

Run `scripts/render-figure.js` per spec to convert it to inline SVG/HTML. Hierarchy is routed to Graphviz `dot` with uniform-width nodes computed from the longest label; sequence, matrix, and table are hand-rolled.

Run `scripts/measure-figure.js` as the deterministic quality gate. The gate parses the rendered output and emits a JSON report of measured numbers and tolerances: sibling-node width equality, horizontal sibling-spacing uniformity, bounding-box overlap count, edge-endpoint snap to border, connector-length equality, label-inside-step, and the no-inline-bolding rule for tables. A figure with any `pass: false` check is rejected and sent back to the spec-author for one corrective pass. If it fails twice, drop the figure.

Read `references/figure-layouts.md` for the family palette and the refused-layout list (`mind-map`, `radial`, `scatter`, `free-form`).

### 8. Render the document as styled HTML, then print to PDF

Run `scripts/build-pdf.js --draft report/draft.md --specs report/figures --out report/<topic-slug>.pdf`. The script:

1. Loads every spec from `report/figures/`, renders each through `render-figure.js`, runs `measure-figure.js`, and aborts if any figure fails the gate.
2. Assembles the markdown draft into HTML, substituting `{{fig:<id>}}` placeholders with the rendered figure HTML.
3. Inlines `assets/print.css`, which enforces the no-gratuitous-bolding rule for tables (header bold; cell bold opt-in only via `emphasis: true`).
4. Launches a headless browser, gates on `document.fonts.ready` and `networkidle0`, and calls `page.pdf({ format: "A4", printBackground: true, preferCSSPageSize: true })`.

After the PDF is written, run `scripts/verify-pdf.js --html <built.html> --pdf <built.pdf>` for post-render checks: figure-fit-content-width, WCAG AA text contrast, and (when `--second-pdf` is supplied) byte-equal determinism after stripping `/CreationDate`, `/ModDate`, and `/ID`.

The PDF path never originates from a non-browser PDF generator. Read `references/render-protocol.md` for the styling defaults, page sizing, and the headless-browser invocation contract.
